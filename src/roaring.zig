...
pub const Bitmap = extern struct {
    ...
    fn checkNewBitmap(bitmap: ?*roaring_bitmap_t) RoaringError!*Bitmap {
        if (bitmap) |b| {
            return conv(b);
        } else {
    ...
    pub fn create() RoaringError!*Bitmap {
        return checkNewBitmap( roaring_bitmap_create() );
    }
    ...
    pub fn createWithCapacity(capacity: u32) RoaringError!*Bitmap {
        return checkNewBitmap( roaring_bitmap_create_with_capacity(capacity) );
    }

    ///
    pub fn free(self: *Bitmap) void {
        roaring_bitmap_free(conv(self));
    }

    ///
    pub fn fromRange(min: u64, max: u64, step: u32) RoaringError!*Bitmap {
        return checkNewBitmap( roaring_bitmap_from_range(min, max, step) );
    }

    ///
    pub fn fromSlice(vals: []u32) RoaringError!*Bitmap {
        return checkNewBitmap( roaring_bitmap_of_ptr(vals.len, vals.ptr) );
    }

    ///
    pub fn getCopyOnWrite(self: *const Bitmap) bool {
        return roaring_bitmap_get_copy_on_write(conv(self));
    }
    ...
    pub fn overwrite(dest: *Bitmap, src: *const Bitmap) bool {
        return roaring_bitmap_overwrite(conv(dest), conv(src));
    }
    ...
    pub fn add(self: *Bitmap, x: u32) void {
        roaring_bitmap_add(conv(self), x);
    }

    ///
    pub fn addMany(self: *Bitmap, vals: []u32) void {
        roaring_bitmap_add_many(conv(self), vals.len, vals.ptr);
    }

    /// Add value x
    /// Returns true if a new value was added, false if the value already existed.
    pub fn addChecked(self: *Bitmap, x: u32) bool {
        return roaring_bitmap_add_checked(conv(self), x);
    }
    ...
    pub fn remove(self: *Bitmap, x: u32) void {
        roaring_bitmap_remove(conv(self), x);
    }

    /// Remove value x
    /// Returns true if a new value was removed, false if the value was not existing.
    pub fn removeChecked(self: *Bitmap, x: u32) bool {
        return roaring_bitmap_remove_checked(conv(self), x);
    }

    /// Remove multiple values
    pub fn removeMany(self: *Bitmap, vals: []u32) void {
        roaring_bitmap_remove_many(conv(self), vals.len, vals.ptr);
    }

    /// Remove all values in range [min, max)
    pub fn removeRange(self: *Bitmap, min: u64, max: u64) void {
        roaring_bitmap_remove_range(conv(self), min, max);
    }

    /// Remove all values in range [min, max]
    pub fn removeRangeClosed(self: *Bitmap, min: u32, max: u32) void {
        roaring_bitmap_remove_range_closed(conv(self), min, max);
    }

    ///
    pub fn clear(self: *Bitmap) void {
        roaring_bitmap_clear(conv(self));
    }

    ///
    pub fn contains(self: *const Bitmap, x: u32) bool {
        return roaring_bitmap_contains(conv(self), x);
    }
    ...
    pub fn containsRange(self: *const Bitmap, start: u64, end: u64) bool {
        return roaring_bitmap_contains_range(conv(self), start, end);
    }

    ///
    pub fn empty(self: *const Bitmap) bool {
        return roaring_bitmap_is_empty(conv(self));
    }
    ...
    pub fn _and(a: *const Bitmap, b: *const Bitmap) RoaringError!*Bitmap {
        return checkNewBitmap( roaring_bitmap_and(conv(a), conv(b)) );
    }

    ///
    pub fn _andInPlace(a: *Bitmap, b: *const Bitmap) void {
        roaring_bitmap_and_inplace(conv(a), conv(b));
    }

    ///
    pub fn _andCardinality(a: *const Bitmap, b: *const Bitmap) u64 {
        return roaring_bitmap_and_cardinality(conv(a), conv(b));
    }

    ///
    pub fn intersect(a: *const Bitmap, b: *const Bitmap) bool {
        return roaring_bitmap_intersect(conv(a), conv(b));
    }

    ///
    pub fn jaccardIndex(a: *const Bitmap, b: *const Bitmap) f64 {
        return roaring_bitmap_jaccard_index(conv(a), conv(b));
    }
    ...
    pub fn _orMany(bitmaps: []*const Bitmap) RoaringError!*Bitmap {
        return checkNewBitmap( roaring_bitmap_or_many(
                    @intCast(u32, bitmaps.len),
                    @ptrCast([*c][*c]const roaring_bitmap_t, bitmaps.ptr)
        ) );
    }

    ///
    pub fn _orManyHeap(bitmaps: []*const Bitmap) RoaringError!*Bitmap {
        return checkNewBitmap( roaring_bitmap_or_many_heap(
                    @intCast(u32, bitmaps.len),
                    @ptrCast([*c][*c]const roaring_bitmap_t, bitmaps.ptr)
        ) );
    }
    ...
    pub fn _xorMany(bitmaps: []*const Bitmap) RoaringError!*Bitmap {
        return checkNewBitmap(roaring_bitmap_xor_many(
                    @intCast(u32, bitmaps.len),
                    @ptrCast([*c][*c]const roaring_bitmap_t, bitmaps.ptr)
        ));
    }
    ...
    pub fn _andnot(a: *const Bitmap, b: *const Bitmap) RoaringError!*Bitmap {
        return checkNewBitmap(roaring_bitmap_andnot(conv(a), conv(b)));
    }
    ...
    pub fn flip(self: *const Bitmap, start: u64, end: u64) RoaringError!*Bitmap {
        return checkNewBitmap(roaring_bitmap_flip(conv(self), start, end));
    }

    ///
    pub fn flipInPlace(self: *Bitmap, start: u64, end: u64) void {
        roaring_bitmap_flip_inplace(conv(self), start, end);
    }
    ...
    pub fn _orLazy(a: *const Bitmap, b: *const Bitmap, convert: bool) RoaringError!*Bitmap {
        return checkNewBitmap(roaring_bitmap_lazy_or(conv(a), conv(b), convert));
    }
    ...
    pub fn _orLazyInPlace(a: *Bitmap, b: *const Bitmap, convert: bool) void {
        roaring_bitmap_lazy_or_inplace(conv(a), conv(b), convert);
    }
    ...
    pub fn _xorLazy(a: *const Bitmap, b: *const Bitmap) RoaringError!*Bitmap {
        return checkNewBitmap(roaring_bitmap_lazy_xor(conv(a), conv(b)));
    }
    ...
    pub fn _xorLazyInPlace(a: *Bitmap, b: *const Bitmap) void {
        roaring_bitmap_lazy_xor_inplace(conv(a), conv(b));
    }
    ...
    pub fn repairAfterLazy(a: *Bitmap) void {
        roaring_bitmap_repair_after_lazy(conv(a));
    }
    ...
    pub fn serialize(self: *const Bitmap, buf: []u8) usize {
        return roaring_bitmap_serialize(conv(self), buf.ptr);
    }

    ///
    pub fn deserialize(buf: []const u8) RoaringError!*Bitmap {
        return checkNewBitmap(roaring_bitmap_deserialize(buf.ptr));
    }

    ///
    pub fn sizeInBytes(self: *const Bitmap) usize {
        return roaring_bitmap_size_in_bytes(conv(self));
    }

    ///
    pub fn portableSerialize(self: *const Bitmap, buf: []u8) usize {
        return roaring_bitmap_portable_serialize(conv(self), buf.ptr);
    }

    ///
    pub fn portableDeserialize(buf: []const u8) RoaringError!*Bitmap {
        return checkNewBitmap(roaring_bitmap_portable_deserialize(buf.ptr));
    }
    ...
    pub fn portableDeserializeSize(buf: []const u8) usize {
        return roaring_bitmap_portable_deserialize_size(buf.ptr, buf.len);
    }

    ///
    pub fn portableSizeInBytes(self: *const Bitmap) usize {
        return roaring_bitmap_portable_size_in_bytes(conv(self));
    }
    ...
    pub fn frozenSizeInBytes(self: *const Bitmap) usize {
        return roaring_bitmap_frozen_size_in_bytes(conv(self));
    }
    ...
    pub fn cardinality(self: *const Bitmap) u64 {
        return roaring_bitmap_get_cardinality(conv(self));
    }
    ...
    pub fn minimum(self: *const Bitmap) u32 {
        return roaring_bitmap_minimum(conv(self));
    }

    ///
    pub fn maximum(self: *const Bitmap) u32 {
        return roaring_bitmap_maximum(conv(self));
    }
    ...
    pub fn printfDescribe(self: *const Bitmap) void {
        roaring_bitmap_printf_describe(conv(self));
    }

    ///
    pub fn printf(self: *const Bitmap) void {
        roaring_bitmap_printf(conv(self));
    }
    ...
    pub fn removeRunCompression(self: *Bitmap) bool {
        return roaring_bitmap_remove_run_compression(conv(self));
    }
    ...
    pub fn shrinkToFit(self: *Bitmap) usize {
        return roaring_bitmap_shrink_to_fit(conv(self));
    }
    ...
    const Iterator = struct {
        ...
        pub fn next(self: *Iterator) bool {
            return roaring_advance_uint32_iterator(&self.i);
        }

        ///
        pub fn previous(self: *Iterator) bool {
            return roaring_previous_uint32_iterator(&self.i);
        }

        ///
        pub fn moveEqualOrLarger(self: *Iterator, x: u32) bool {
            return roaring_move_uint32_iterator_equalorlarger(&self.i, x);
        }
    ...
    pub fn iterator(self: *const Bitmap) Iterator {
        ...
        roaring_init_iterator(conv(self), &ret.i);
        return ret;
    }
};
